...
### Added
- feat DIALER-001: Power Dialer mockup at /dialer. Queue builder (filter by stage/state/owner/activity/surplus, configure caller ID + dialing mode, scrollable preview with remove/restore), three-panel active call layout (lead context with all contacts/numbers + scrollable notes feed; centered call surface with phase animation, timer, mute/hold/end/try-next; right-hand quick note + stage change + follow-up email/text). End Call opens a disposition modal with eight outcomes. 24 in-memory test leads across GA/SC/TN/PA/OH/NY mixed living/deceased with disconnected/wrong numbers and seeded notes. Sidebar gains a "Dialer" entry below Leads. Reports > Dialer placeholder added. Settings > Dialer subgroup added with Phone Numbers, Preferences, and Dispositions panels (mock data). No telephony, staging only, not part of the July 1 beta. (2026-06-15T01:00:00-05:00)
