...
- Fix KANBAN-SCROLL: Leads board uses one scroll context across both axes (revised from the per-column scroll shipped earlier in this Unreleased block, which felt wrong because each column had its own vertical bar). Outer `kanban-scroll` div is `flex-1 min-h-0 overflow-auto`, each `KanbanColumn` header is `sticky top-0 z-10 rounded-t-lg` so stage names + counts stay pinned when the operator scrolls vertically, and the inner card body drops back to its natural height (`min-h-[460px]`) without its own scroll container. The sticky header now carries a `shadow-[0_-1px_0_0_#e5e7eb]` (and `#13644e` on drag-hover) so when the column's wrapper border scrolls out of view, the header keeps a visible top edge instead of looking decapitated. Scrollbar treatment anchored to Linear / Attio / Pipedrive: thin neutral (Chrome `#cbd5e1` thumb on transparent track, hover `#94a3b8`; Firefox `scrollbar-color: #cbd5e1 transparent`) on both axes. Dropped the Fix V petrol pill, brand-colored scrollbars are atypical in peer CRMs and read as decoration competing with the cards. (2026-06-19T22:45:00-05:00)
