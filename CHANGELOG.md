...
### Fixed
- Fix COOKIE-CONSENT: Banner no longer flashes on every server-rendered page. Root cause: `useSyncExternalStore` returned `null` from the server snapshot so SSR markup always included the banner, then hydration read localStorage and hid it, causing a visible flicker on each full navigation (login redirect, page reload, hard nav). Rewrote `CookieConsent.tsx` to start `show=false` and flip to `true` only inside `useEffect` when no consent is found in localStorage. After one OK click, every subsequent mount finds the stored value and never re-shows the banner. (2026-06-19T23:30:00-05:00)
...
- Fix KANBAN-SCROLL: Leads board uses one scroll context across both axes (revised from the per-column scroll shipped earlier in this Unreleased block, which felt wrong because each column had its own vertical bar). Outer `kanban-scroll` div is `flex-1 min-h-0 overflow-auto`, each `KanbanColumn` header is `sticky top-0 z-10 rounded-t-lg` so stage names + counts stay pinned when the operator scrolls vertically, and the inner card body drops back to its natural height (`min-h-[460px]`) without its own scroll container. The petrol `.kanban-scroll` styling grew a `width: 8px` rule + matching `::-webkit-scrollbar-corner` so the vertical bar is the same petrol-on-slate treatment as the horizontal bar instead of the default Chrome gray. Both bars are anchored to the right and bottom edges of the board area. (2026-06-19T22:30:00-05:00)
