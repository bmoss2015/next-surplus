...
### Fixed
- Fix MAIL-ADDR-PERSIST: USPS-corrected address now persists to the contact row so the next send to the same recipient doesn't ask for the correction again. New `persistContactAddress` server action in `src/app/(app)/mail/_verify-action.ts` rewrites `contacts.value` with the canonical "line1, city, ST ZIP" form via `formatAddressForStorage()`. `SendMailModal.applyAddressSuggestion()` calls it whenever the operator clicks Use Corrected Version, then re-runs `verifyAddressAction` with the contact_id and `force: true` so the verify cache repopulates against the new value. The existing BEFORE UPDATE trigger on `public.contacts` (migration 0133) clears the `address_verify_*` cache fields automatically when value changes, so the new verify run hits the provider once and stores the fresh result. (2026-06-19T16:00:00-05:00)
