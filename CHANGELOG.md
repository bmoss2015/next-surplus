...
### Fixed
- Fix MAIL-THUMB-BW: Mail Sent thumbnail in lead detail now respects the B&W color flag. `LetterThumbnail` accepts an optional `color` prop and applies a `grayscale(100%)` CSS filter on the rendered iframe and PDF first-page image when the piece was sent in black and white. Caller in `LeadMailV11Client.tsx` threads `piece.color` through. Previously the thumbnail rendered in full color regardless of how the letter was sent, contradicting the same grayscale treatment that the full Letter Preview modal and the Mail tab inline thumbnail already applied. (2026-06-19T15:00:00-05:00)
