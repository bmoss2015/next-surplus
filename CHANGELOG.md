...
### Removed
- Fix LOB-SCRAPER-DROP: Removed the weekly Lob wholesale-pricing scraper at `/api/cron/lob-pricing-sync` and its helper `src/lib/mail/fetch-lob-pricing.ts`. The scraper hit `help.lob.com/llms-full.txt` and parsed eight rate lines with regex; Lob updated their published page format and every pattern began failing, emitting three `lob_pricing_sync_failed` rows into the in-portal `notifications` bell each Monday at 04:00 UTC. Customer-facing bell is multi-tenant SaaS, so ops health alerts have no business surfacing there. Wholesale rates are pinned to the `LOB_PUBLISHED_DEVELOPER_PRICING` constant in `src/lib/mail/lob.ts` (Lob Developer-tier published rates verified 2026-05-22). The DB column `app_pricing_config.wholesale_pricing_cents` already mirrors the constant, so mail send cost calc is unchanged. When Lob next changes published rates, update the constant and run a one-line UPDATE on the column in the same PR. Provider Costs panel empty-state copy updated to reflect the new manual update model. Cron entry removed from `vercel.json`. (2026-06-22T11:00:00-05:00)
