...
### Removed
- Fix MAIL-FONTS-PERMISSIVE: Dead font-allowlist system removed from the Mail Templates pipeline. Deleted `src/lib/mail/fonts.ts` (which defined a `SUPPORTED_FONTS` allowlist + `isFontSupported()` gate) and dropped the unused `validateTemplateFonts` and `validateDocxPathFonts` server actions in `src/lib/mail/actions.ts`. Nothing in the codebase was calling these, they were leftover from an earlier strict-allowlist plan. The Gotenberg / LibreOffice headless renderer continues to substitute any missing font with a metric-compatible match silently (Carlito for Calibri, Caladea for Cambria, Liberation Sans for Arial, etc.), so docx templates with any font now upload and render without any "this font isn't installed" gate. (2026-06-19T15:30:00-05:00)
