...
### Added
- Fix AFFILIATE-REF-V1: Adds ?ref= attribution to signup. Signup page reads the ref query param on mount, normalizes it (uppercase, 32 char cap, A-Z0-9_-), and stores it in sessionStorage so a user who lands on /signup?ref=NICK30 still gets credited after navigating around the landing pages. Signup server action accepts the ref, sets it as `affiliate_ref` on the Stripe Checkout Session metadata + subscription metadata (so Stripe-side reporting can attribute the org and every renewal), and when the ref maps to a known affiliate coupon, swaps `allow_promotion_codes` out for `discounts: [{ coupon }]` to auto-apply the discount with no manual code entry. NICK30 maps to coupon id `NICK30` (30% off, 12-month repeating). New script `scripts/create-affiliate-coupons.mjs` is a one-time idempotent Stripe call that creates the coupon + promotion code from the canonical list in the script itself. Signup form shows a neutral Discount Applied chip when a ref is present so the user can see the discount is in effect before they hit Stripe Checkout. (2026-06-26T11:50:00-05:00)
...
### Fixed
- Fix SET-PRIMARY-AUTO-AND-AWAIT: Two follow-ups on the Primary feature from PR #218. (1) On successful verification, if no other bank in the org is currently primary, auto-set the just-verified bank as primary. Matches Stripe / Mercury default-payment-method behavior. Without this the first verified bank would sit there without a Primary chip until the operator manually clicked Set As Primary even though there was nothing to compete with. (2) Set As Primary button's useTransition wrapper now awaits router.refresh() so the loading state resolves only after the server re-render completes. Previously the transition resolved before the refresh finished, occasionally leaving the button text stuck on "Setting…" until the user manually refreshed. (2026-06-26T09:00:00-05:00)
- Fix VERIFY-CARD-SORT-AND-ATTEMPTS-COPY: Two fixes on Settings > Bank Accounts. (1) Sort verified accounts ahead of unverified in fetchMailBankAccounts, then newest-first within each group. Matches Stripe Dashboard / Mercury / Bill.com pattern. Previously a verified account added before any unverified ones would render in the middle of the list. (2) Reworked the attempts-remaining copy in the Verify Manually modal to drop "verification" (redundant inside a modal titled Verify Bank Account) and group the "N of 3" as a single bolded unit. Resolves the "3 v" kerning issue where the final digit was visually colliding with the v in "verification". (2026-06-26T08:35:00-05:00)
- Fix VERIFY-MODAL-UI-POLISH: Four polish fixes on the Verify Manually modal. (1) Sample bank statement preview no longer shows a fake-looking `SMABCD` code which read as a real prefix users should look for; replaced with `SM••••` dot-mask chip and bumped the suffix to `-LOB.COM` to match the real descriptor Lob writes. (2) Verification Code input placeholder changed from `SM____` (read as "SM is pre-filled, type the rest") to `Example: SMTJDY` plus a clarifying helper line below the input ("Type all 6 characters including SM"). (3) Sample preview spacing increased (padding 10px→14px, row gap 6px→10px, column gap 8px→12px, column widths slightly wider) so the rows breathe. (4) Attempt counter "3 of 3" rendered with `font-variant-numeric: tabular-nums` so the "3" no longer collides with the surrounding "v" in "of". (2026-06-25T18:45:00-05:00)
...
### Changed
- Fix VERIFY-BANK-MODE-AWARE-CARD: Move mode-specific verification copy out of the Add Bank drawer (where we don't yet know the bank's microdeposit_type) and onto the bank card itself (where we do). Drawer hint now reads "Verification instructions appear on the bank card after this account is added." Bank card under VERIFYING badge now shows a hint with the right copy per mode: "Two small test deposits arrive in 1 to 2 business days" for amounts banks, "A single $0.01 deposit with a 6 character verification code starting with SM arrives in 1 to 2 business days" for descriptor_code banks. Removes presumptuous pre-submit copy that promised a specific verification process before Lob had assigned a mode. (2026-06-25T16:30:00-05:00)
- Fix VERIFY-BANK-VISUAL-EXAMPLES: Adds a stylized mock-bank-statement preview to the Verify Manually modal so customers can see at a glance what to look for in their actual statement. For descriptor_code mode the preview shows one ACH CREDIT row with the 6 character SM-prefixed code rendered as a brand-colored chip. For amounts mode the preview shows two LOB-VERIF rows with the cent amounts in brand color. No images, just inline divs styled with var(--brand) / var(--hairline) / var(--surface) tokens. Replaces the text-only "open the transaction in your bank statement and read the descriptor" instructions which were correct but easy to misread. (2026-06-25T15:30:00-05:00)
- Fix LOB-MICRODEPOSIT-DUAL-MODE: Reverses the fintech-rejection guard from LOB-REJECT-DESCRIPTOR-BANK in favor of an auto-detecting verify modal that supports both Lob micro-deposit modes. After bank add, query Lob for `microdeposit_type` and store it on `mail_bank_accounts.microdeposit_type` (new column, migration 0161). The Verify Manually modal branches its UI from that field: traditional banks render two cent inputs ("two deposits arriving in 1-2 business days"), fintech banks (Mercury, Brex, Novo, Rho, Ramp) render a single 6-character code input ("single $0.01 deposit with a code starting with SM in the ACH descriptor"). Customer never sees an error or a bank-type message; the right form just appears. New `verifyMailBankAccountWithCode` server action handles the descriptor_code variant. `lobVerifyBankAccount` helper now accepts a `LobVerifyInput` union. Add-bank drawer help text updated to be mode-agnostic. (2026-06-25T12:00:00-05:00)
...
### Removed
- Fix PLAID-CLEANUP: Removed all dead Plaid integration code. The Plaid Link UI was pulled out of the Add Bank drawer in an earlier round (typing routing/account is simpler than guiding through an OAuth dance for the same result, and Lob does not accept Plaid processor tokens). What remained was a server action `exchangePlaidPublicTokenForBankAccount`, a `createPlaidLinkToken` server action, a `plaid_microdeposit_poll` cron that ran every 4 hours and was a no-op (zero rows with `plaid_access_token`), the `@/lib/plaid/client` module, and `plaid` + `react-plaid-link` package dependencies. All gone now. The DB columns `mail_bank_accounts.plaid_access_token` and `plaid_item_id` and the `verified_via` enum value `plaid` are intentionally retained (no row references them; dropping would require a migration with negligible benefit). Comment on `verifyMailBankAccountManually` updated to remove the stale "cron picks up the rest" framing. (2026-06-22T12:45:00-05:00)


### Fixed
- Fix COOKIE-CONSENT: Banner no longer flashes on every server-rendered page navigation. Root cause: `useSyncExternalStore` returned `null` from the server snapshot, so SSR markup always included the banner and hydration hid it after the fact, causing a visible flicker on every full render (login redirect, page reload, hard nav). Rewrote `CookieConsent.tsx` so the server snapshot returns the consented sentinel (`"essential-only"`) — SSR markup never contains the banner. After hydration, the client snapshot reads localStorage; if no consent is stored, the banner appears for the first time. Dismiss writes localStorage and pings a module-level listener set so the snapshot re-reads in the same tab (`window.storage` events don't fire same-tab). (2026-06-19T23:30:00-05:00)
- Fix BANK-MANUAL-ENTRY: Add Bank Account drawer now has a manual routing + account number entry mode that bypasses Plaid Link. Used when Plaid env vars are misconfigured (operator was hitting "client_id must be a properly formatted, non-empty string" on prod with no entry fallback) and for operators who prefer typing. New `addMailBankAccountManually` server action validates routing (9 digits) + account (4-17 digits) + holder, sends to Lob `/bank_accounts`, inserts the row with `verified_via='micro_deposits'`. Standard micro-deposit cycle then runs and the existing Verify Manually modal on the bank card handles the final verification step. Toggle link at the bottom of the drawer flips between Connect Bank (Plaid) and Enter Manually. (2026-06-19T18:00:00-05:00)
...
- Fix KANBAN-SCROLL: Leads board uses one scroll context across both axes (revising the per-column scroll that originally shipped under this label, which felt wrong because each column had its own vertical bar). Outer `kanban-scroll` div is `flex-1 min-h-0 overflow-auto`, each `KanbanColumn` header is `sticky top-0 z-10 rounded-t-lg` so stage names + counts stay pinned when the operator scrolls vertically, and the inner card body drops back to its natural height (`min-h-[460px]`) without its own scroll container. The column wrapper drops `border` for `border-x border-b` and the sticky header carries its own `border-t` (gray-200, petrol-500 on drag-hover) so the top edge is part of the sticky element itself, not a wrapper attribute that scrolls out of view (a box-shadow attempt got clipped by the parent's `overflow-auto`). Scrollbar treatment anchored to Linear / Attio / Pipedrive — thin neutral (Chrome 8px, `#d4d4d4` thumb on transparent track, hover `#a3a3a3`; Firefox `scrollbar-color: #d4d4d4 transparent`) on both axes. Switched from Tailwind slate (`#cbd5e1` / `#94a3b8`), which had a perceptible blue cast, to true achromatic neutral. Dropped the Fix V petrol pill — brand-colored scrollbars are atypical in peer CRMs and read as decoration competing with the cards. (2026-06-19T23:00:00-05:00)
- Fix BANK-VERIFY-AUDIT: Full audit and hardening of the Lob micro-deposit verification flow. (1) New Verify Manually modal on every unverified bank card lets the operator enter the two cent amounts from their bank statement directly, called via new `verifyMailBankAccountManually` server action that hits Lob `/bank_accounts/{id}/verify` and flips `mail_bank_accounts.status` to `verified` on success. (2) Cron `/api/cron/plaid-microdeposit-poll` now filters Plaid transactions by merchant name (only credits whose `name`/`merchant_name` contains "LOB" qualify), preventing the cron from feeding random sub-$1 credits like interest accrual into Lob `/verify` and burning the 3-attempt limit. (3) New `verify_attempts` + `last_verify_error` + `last_verify_attempt_at` columns on `mail_bank_accounts` (migration 0155). Cron caps auto-attempts at 2 so the operator always retains the third attempt manually. (4) Vercel cron cadence increased from `0 9 * * *` (daily) to `0 */4 * * *` (every 4 hours) so verification completes faster after the second deposit posts. (5) Bank card UI shows last-checked relative time, attempt counter, and Lob error message inline. Drawer copy rewritten to explain the timing and manual-verify fallback. (2026-06-19T17:00:00-05:00)
...
- Fix 15: Signup / Login / Verify / Onboarding shipping the four customer acquisition pages for the July 1, 2026 public beta. Adds `/signup-mockups` and `/login-mockups` galleries with 10 anchored variants (5 each, mix of split and centered layouts) under a new `(mockups)` route group, gated to non-production hosts via the layout chrome. Anchors cited per variant: Linear (linear.app), Attio (attio.com), Folk (folk.app), Pipedrive (pipedrive.com), Vercel (vercel.com), Stripe Dashboard (dashboard.stripe.com), Supabase (supabase.com/dashboard). Real production `/signup` page wired to a new `signUp` server action that creates the Supabase user, provisions the org + owner profile rows, signs the user in, and creates a Stripe Checkout Session at the `beta_founder` monthly price with `client_reference_id` and `metadata.org_id` set per the P1b webhook fix. `/signup/verify` polls a new `/api/billing/subscription-status` endpoint every 2s, renders Loading / Success / Stalled states (stalled at 30s) and auto-redirects on subscription `active` or `trialing` to `/onboarding/firm`. New 4-step `/onboarding/[step]` wizard (Firm / Import / Inbox / Team) with skip allowed on steps 2-4, progress dots, equal-width footer CTAs, and Plus Jakarta Sans Medium loaded via Google Fonts per the brand pack. Onboarding inbox step links to existing `/api/oauth/google/start` for Gmail and surfaces a non-yellow notice about the Google verification warning. (2026-06-17T00:00:00-05:00)
...
### Added
- Fix 10d: Strip Outlook + "coming next" language from landing v1. Feature card 02 title changes from "Gmail And Outlook, Sorted To The Right Lead" to "Gmail, Sorted To The Right Lead" because Outlook + IMAP inbox sync are not QA-validated yet. Removes the gray subtitle below the pricing card that read "Microsoft Outlook and IMAP inbox sync are coming next." which violated the no "coming soon" rule for the landing page. (2026-06-16T22:15:00-05:00)
