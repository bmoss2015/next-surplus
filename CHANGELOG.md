...
### Added
- Fix 10c: Landing v1 FAQ rewrite v5. Cuts the FAQ from nine questions to seven and reframes every remaining question against Pipedrive, HubSpot, Monday CRM, Close, and Copper FAQ patterns. Drops every Q whose answer is already visible in the pricing card or feature grid above (Who Built This, Is The Development Team Active, How Much Does Next Surplus Cost, Is There A Free Trial, What Is Included). Adds How Quickly Can I Get Started, Can I Import Leads From My Current System, Are There Per Seat Fees Or Add Ons, and What Kind Of Support Do You Offer. Reframes Will My Existing Gmail Account Work, How Is My Data Protected, and Can I Cancel My Subscription. Title Case headers throughout per design rules; no "coming soon" language. (2026-06-16T21:00:00-05:00)
- Fix 13: Stripe billing integration. New `/api/stripe/create-checkout-session` creates a subscription Checkout Session at the price ID matching the org's `plan_tier` (founder, beta_founder monthly/annual, or standard) with a 14 day trial. `/api/webhooks/stripe` verifies `STRIPE_WEBHOOK_SECRET` and handles `customer.subscription.created/updated/deleted`, `invoice.paid`, and `invoice.payment_failed`, writing `stripe_customer_id`, `stripe_subscription_id`, `subscription_status`, and `last_invoice_paid_at` to `orgs`. `/api/stripe/create-portal-session` hands customers off to the Stripe Customer Portal. A new daily cron at `/api/cron/founder-lock-expiration` (12:00 UTC) emails beta_founder admins at the 10 month mark, then swaps the Stripe subscription to the standard monthly price and flips `plan_tier` to `standard` at the 12 month mark. Migration 0151 adds the Stripe columns + unique partial indexes; vercel.json registers the new cron. (2026-06-16T20:00:00-05:00)
