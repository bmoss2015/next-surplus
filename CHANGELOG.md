...
- Fix KANBAN-SCROLL: Leads board now owns its own scroll context so mouse-wheeling on a column scrolls only that column's cards instead of the whole page. Outer board container becomes `flex h-full flex-col` with `flex-1 min-h-0`, the horizontal scroll strip switches to `overflow-x-scroll overflow-y-hidden` so the petrol scrollbar stays anchored to the bottom of the board at all times, and each `KanbanColumn` is now `flex h-full flex-col` with a `shrink-0` header and a `flex-1 min-h-0 overflow-y-auto overscroll-contain` card body. `overscroll-contain` prevents scroll chaining into the page when a column hits its top or bottom. Stage headers and counts stay pinned while operators scan tall stages like Outreach / Negotiating. (2026-06-19T22:00:00-05:00)
