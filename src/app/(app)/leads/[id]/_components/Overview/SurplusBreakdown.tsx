...
export function SurplusBreakdown({
  ...
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-6 shadow-[0_1px_3px_rgba(0,0,0,0.08)]">
      <h3 className="mb-4 text-[11px] font-bold uppercase tracking-[0.08em] text-[#0d4b3a]">
        Surplus Breakdown
      </h3>
      ...
      <div>
        <div className="flex items-center justify-between gap-4">
          <span
            className={cn(
              ...
              hasConfirmed
                ...
                : "text-sm font-semibold text-[#13644e]"
            )}
          >
            Potential Surplus
          </span>
          <span
            className={cn(
              hasConfirmed ? "text-base font-normal text-[#9ca3af]" : "text-xl font-bold text-[#0d4b3a]"
            )}
          >
        ...
        <div className="flex items-center justify-between gap-4">
          <span
            className={cn(
              ...
              hasConfirmed ? "font-semibold text-[#13644e]" : "font-medium text-[#6b7280]"
            )}
          >
            Confirmed Surplus
          </span>
          {hasConfirmed && (
            <span className="text-xl font-bold text-[#0d4b3a]">{formatCurrency(confirmedSurplus)}</span>
          )}
        </div>
      ...
      <hr className="my-5 border-t-2 border-[#13644e]" />

      {/* EST. NET PAYOUT — the headline number */}
      <div className="flex items-center justify-between gap-4">
        <span className="text-base font-bold tracking-wide text-[#0d4b3a]">
          Estimated Net Payout
        </span>
        <span className="text-xl font-bold text-[#0d4b3a]">{formatCurrency(netPayout)}</span>
      </div>
