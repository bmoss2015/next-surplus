    ...
    <div className="min-h-screen bg-white">
      <div className="mx-auto max-w-5xl px-8 py-8">
        ...
        <section>
          ...
          {/* Same card layout, scoped to this lead */}
          <div className="space-y-4">
            <Card p={SAMPLE[0]} />
            <Card p={SAMPLE[1]} />
            <Card p={SAMPLE[3]} />
...
function Card({ p }: { p: (typeof SAMPLE)[number] }) {
  ...
  return (
    <article className="grid grid-cols-[160px_1fr] gap-6 rounded-lg border border-gray-200 bg-white p-5">
      {/* Letter thumbnail */}
      <div className="relative h-[200px] overflow-hidden rounded-sm border border-gray-200 bg-white">
        <div className="absolute inset-0 p-3 text-[7px] leading-[1.5] text-ink">
          <div className="mb-2 text-right text-gray-500">{p.sentAt}</div>
          ...
          <div className="mb-1 text-gray-700">
            ...
            being held by the county following the recent sale of property
            at...
          </div>
          <div className="mb-1 text-gray-700">
      ...
      <div className="flex min-w-0 flex-col">
        <div className="flex items-start justify-between gap-3">
          ...
          <span
            className={`inline-flex items-center justify-center rounded-[4px] px-[10px] py-[5px] text-[9.5px] font-semibold uppercase leading-none tracking-[0.12em] ${sc.bg} ${sc.text}`}
          >
            {p.status}
          </span>
        </div>
        ...
        <dl className="mt-4 grid grid-cols-2 gap-x-6 gap-y-3 text-[12px]">
          ...
          {p.status === "In Transit" && (
            <KV label="ETA">2-4 business days</KV>
          )}
          {/* Tracking on a single line — col-span-2 so it never wraps */}
          <div className="col-span-2">
            <KV label="Tracking">
              <a
                href="#"
                className="cursor-pointer whitespace-nowrap font-mono text-[11.5px] text-[#0d4b3a] underline decoration-[#0d4b3a]/30 underline-offset-2 hover:decoration-[#0d4b3a]"
              >
                {p.tracking}
              </a>
            </KV>
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 text-[12px] font-medium">
