...
export default async function MailReportPage() {
  const { months, totals } = await fetchMailReport({ months: 12 });
    ...
    <div className="px-7 py-6">
      ...
      <div className="mb-[22px]">
        ...
        <div className="mt-1 text-[13px] text-gray-500">
          Monthly volume and spend across all physical mail sent through
          the portal. Last 12 months.
        </div>
      </div>

      <div className="mb-5 grid grid-cols-4 gap-3">
        <StatCard label="Sent" value={String(totals.sent_total)} sub="12-Month Total" />
        <StatCard label="Delivered" value={String(totals.delivered)} sub={`${rate(totals.delivered, totals.sent_total)}% Delivery Rate`} />
        <StatCard label="Returned" value={String(totals.returned)} sub={`${rate(totals.returned, totals.sent_total)}% Return Rate`} warn={totals.returned > 0} />
        <StatCard label="Spent" value={fmtUSD(totals.spent_cents)} sub="Provider Charges" />
      </div>

      {/* Monthly bars — pure CSS, no chart library. Each bar's height
          scales to the max month so visual comparison stays meaningful
          even with tiny absolute volumes. */}
              ...
              <div key={m.month} className="flex flex-col items-center gap-1">
                <div className="relative flex w-full flex-1 items-end">
                  <div
                    ...
                    style={{ height: `${Math.max(heightPct, m.spent_cents > 0 ? 4 : 0)}%` }}
                    title={`${m.label} · ${fmtUSD(m.spent_cents)}`}
                  />
                </div>
                <div className="text-[10px] text-gray-500">{m.label.split(" ")[0]}</div>
              </div>
            );
          })}
        </div>
        ...
        <table className="w-full">
          ...
          <tbody>
            {months
              ...
              .reverse()
              .map((m) => (
                <tr key={m.month} className="border-b border-gray-150 last:border-b-0 text-[12.5px]">
