...
export default async function V40Page() {
  if (process.env.VERCEL_ENV === "production") notFound();
  const lead = ACTIVE_LEAD;
  return (
    <div className="flex h-screen flex-col bg-[#f4f6f8] text-ink">
      <TopBar />
      <main className="grid flex-1 grid-cols-[280px_minmax(0,1fr)_320px] gap-4 overflow-hidden px-5 py-4">
        <LeftStack />
        <CenterStack />
  ...
  return (
    <header className="flex shrink-0 items-center justify-between border-b border-black/[0.06] bg-white px-7 py-2.5 text-[12px]">
      <div className="flex items-center gap-3">
        <Link href="/share/dialer-mockup" className="text-ink/55 hover:text-ink">← Back</Link>
...
function StatsCard() {
  return (
    <section className={`p-5 ${CARD}`}>
      <div className="text-[10.5px] uppercase tracking-[0.6px] text-ink/55">Session Stats</div>
      <div className="mt-2 grid grid-cols-2 gap-3">
        <StatBlock label="Dials" value="42" />
        <StatBlock label="Connects" value="6" />
        <StatBlock label="Connect Rate" value="14%" />
        <StatBlock label="Avg Talk" value="3:48" />
      </div>
    </section>
  );
}
function StatBlock({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-[10px] uppercase tracking-[0.5px] text-ink/55">{label}</div>
      <div className="text-[26px] font-semibold tabular-nums leading-none text-ink">{value}</div>
    </div>
  );
}
function Queue() {
    ...
    <article className="relative flex flex-col overflow-hidden rounded-2xl p-7 text-white" style={{ background: "linear-gradient(140deg, #04261c 0%, #0d4b3a 60%, #14644e 100%)", boxShadow: "0 24px 50px -22px rgba(13,75,58,0.45), 0 4px 14px -4px rgba(15,23,41,0.08)" }}>
      ...
      <div className="mt-6 flex items-center gap-6">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h1 className="m-0 mt-1 text-[40px] font-semibold leading-[1.02] tracking-[-0.025em] text-white">Cornelius J. Hayes Jr.</h1>
        ...
        <div className="text-right">
          ...
          <div className="mt-1 text-[52px] font-semibold tabular-nums tracking-[-0.025em] leading-none text-white">02:14</div>
        </div>
      </div>
      <div className="mt-auto pt-7">
        <div className="flex items-center gap-1.5 rounded-2xl bg-white/[0.08] p-1.5 backdrop-blur">
          <SoftBtn label="Mute" />
          <SoftBtn label="Hold" />
          <SoftBtn label="VM Drop" />
          <SoftBtn label="Add Note" />
...
function SoftBtn({ label }: { label: string }) {
  return <button className="flex-1 rounded-xl bg-transparent px-3 py-2.5 text-[13px] font-medium text-white hover:bg-white/10">{label}</button>;
}
function NextCall() {
  return (
    <article className={`flex items-center gap-4 p-5 ${CARD}`}>
      <div className="flex flex-col">
        <span className="text-[10.5px] uppercase tracking-[0.6px] text-petrol-700">Next Call · Dialing In</span>
        <span className="mt-0.5 text-[32px] font-semibold tabular-nums leading-none text-ink">0:03</span>
      </div>
      <div className="ml-3 h-12 w-px bg-black/[0.06]" />
      <div className="flex items-center gap-3">
        <div className="flex h-12 w-12 items-center justify-center rounded-full bg-petrol-700 text-[13px] font-semibold text-white">OC</div>
        <div className="min-w-0">
          <div className="text-[10.5px] uppercase tracking-[0.5px] text-ink/55">Lead 4 of 10 · Primary Owner</div>
          <div className="text-[15px] font-semibold leading-tight text-ink">Otis Crockett</div>
          <div className="text-[11.5px] text-ink/65">
            ...
            <span className="tabular-nums">(865) 555-0148</span> · Knoxville, TN · {fmtMoney(76300)}
          </div>
        </div>
      </div>
...
function DealFacts({ lead }: { lead: typeof ACTIVE_LEAD }) {
  return (
    <section className={`p-5 ${CARD}`}>
      ...
      <div className="mt-1 text-[12.5px] text-ink/65">{lead.propertyAddress}, {lead.city}, {lead.state}</div>
      <div className="mt-4 grid grid-cols-2 gap-3 border-y border-black/[0.06] py-4">
        <div>
          ...
          <div className="text-[22px] font-semibold tabular-nums tracking-[-0.02em] leading-tight text-ink">{fmtMoney(lead.estimatedSurplus)}</div>
        </div>
        <div>
          <div className="text-[10px] uppercase tracking-[0.5px] text-petrol-700">Net To Firm</div>
          <div className="text-[22px] font-semibold tabular-nums tracking-[-0.02em] leading-tight text-petrol-500">{fmtMoney(lead.estimatedNet)}</div>
          <div className="mt-0.5 text-[10px] text-ink/55">{lead.recoveryFeePercent}% recovery</div>
        </div>
      </div>
...
function Fact({ label, value }: { label: string; value: string }) {
  return (<div><dt className="text-[10px] uppercase tracking-[0.5px] text-ink/55">{label}</dt><dd className="m-0 mt-0.5 text-[12.5px] font-medium text-ink">{value}</dd></div>);
}
