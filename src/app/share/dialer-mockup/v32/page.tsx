...
export default async function V32Page() {
  ...
  return (
    <div className="flex h-screen flex-col bg-[#f4f6f8] text-ink">
      <TopBar />
      <main className="grid flex-1 grid-cols-12 gap-5 overflow-hidden px-6 py-5">
        <Hero className="col-span-8" />
        <RightRail className="col-span-4" lead={lead} />
      </main>
    </div>
  );
}
...
function Hero({ className }: { className?: string }) {
  return (
    ...
    >
      <span
        className="pointer-events-none absolute -right-12 -top-12 select-none text-[18rem] font-bold leading-none text-white/[0.04]"
      ...
      <div className="relative mt-6 flex items-center gap-7">
      ...
      <div className="relative mt-6">
        ...
        <div className="mt-3 grid grid-cols-5 gap-2">
          ...
          <Outcome label="Not Interested" sub="Stay in this stage" />
      ...
      <div className="relative mt-auto pt-6">
        <div className="text-[10.5px] uppercase tracking-[0.6px] text-petrol-300">
          Quick Note (saves on outcome or when next call dials)
        </div>
        <textarea
          rows={2}
          placeholder="Capture what was said. Skip if there's nothing to add."
          defaultValue=""
...
const CARD = "rounded-2xl border border-black/[0.06] bg-white shadow-[0_8px_28px_-18px_rgba(15,23,41,0.12)]";

function DealCard({ lead }: { lead: typeof ACTIVE_LEAD }) {
  return (
    <section className={`${CARD} p-5`}>
      ...
      <div className="mt-3 grid grid-cols-2 gap-3">
        ...
        <KV label="Net To Firm" value={fmtMoney(lead.estimatedNet)} accent />
      </div>
  ...
  return (
    <div>
      ...
      <div
        className={
          accent
            ? "text-[20px] font-semibold tabular-nums tracking-[-0.02em] leading-tight text-petrol-500"
            : "text-[20px] font-semibold tabular-nums tracking-[-0.02em] leading-tight text-ink"
        }
      >
        {value}
      </div>
...
function ContactsCard({ lead }: { lead: typeof ACTIVE_LEAD }) {
  return (
    <section className={`overflow-y-auto ${CARD} p-5`}>
      <div className="flex items-baseline justify-between">
        <div className="text-[10.5px] uppercase tracking-[0.6px] text-ink/55">
          Contacts and Phones
        </div>
