...
export const QUEUE_SCROLL =
  "[scrollbar-width:thin] [scrollbar-color:#cbd5e1_transparent] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-[#cbd5e1] [&::-webkit-scrollbar-thumb]:rounded-full";
...
export function RightPanel({ lead, overlay }: { lead: Lead; overlay?: "timeline" }) {
  ...
  return (
    <div className={`flex h-full flex-col overflow-hidden rounded-[12px] bg-white ${CARD_SHADOW}`}>
      ...
      <PanelSection title="Latest Activity" trailing={<TimelineLink />}>
      ...
      >
        <div className={`max-h-[280px] overflow-y-auto pr-1 ${QUEUE_SCROLL}`}>
...
function TimelinePanel({ lead }: { lead: Lead }) {
  return (
    <div className={`flex h-full flex-col overflow-hidden rounded-[12px] bg-white ${CARD_SHADOW}`}>
      ...
      <div className={`flex flex-1 flex-col gap-0 overflow-y-auto p-4 ${QUEUE_SCROLL}`}>
        {lead.activity.map((a, i) => (
          <TimelineRow key={`${a.when}-${i}`} activity={a} last={i === lead.activity.length - 1} />
        ))}
      </div>
    </div>
  );
...
function TimelineRow({ activity, last }: { activity: Activity; last?: boolean }) {
  return (
...
export function QueueSearch() {
  return (
    <div
      className="flex h-9 items-center rounded-lg px-3 ring-0 focus-within:ring-1 focus-within:ring-petrol-700"
...
export function QueueList({ items }: { items: QueueItem[] }) {
  return (
    <div className={`flex flex-1 flex-col gap-1 overflow-y-auto pr-1 ${QUEUE_SCROLL}`}>
      {items.map((q, i) => (
